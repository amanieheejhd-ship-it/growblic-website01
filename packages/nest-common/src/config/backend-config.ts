...
export type LoadBackendConfigOptions = {
  /** Development/test fallback for BACKEND_PORT (per-service default). */
  defaultPort?: string;
};
...
export function loadBackendConfig(
  ...
  const configuredPort = values.PORT?.trim();
  const backendPort = values.BACKEND_PORT?.trim();
  const port =
    configuredPort ||
    backendPort ||
    (environment !== "production"
      ? options.defaultPort ?? LOCAL_DEFAULTS.BACKEND_PORT
      : configurationError("BACKEND_PORT"));

  return {
    environment,
    host,
    port: boundedInteger(port, configuredPort ? "PORT" : "BACKEND_PORT", 1, 65_535),
    logLevel,
