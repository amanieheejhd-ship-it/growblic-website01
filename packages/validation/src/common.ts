...
export function readString(
  ...
  if (value === undefined || value === null || value === "") {
    if (required) throw new FormValidationError();
    return null;
  }

  if (typeof value !== "string") throw new FormValidationError();
...
export function readEmail(input: Record<string, unknown>, key: string, required = false) {
...
export function readSubmissionKey(input: Record<string, unknown>) {
  ...
  if (!value || !/^[a-zA-Z0-9_-]+$/.test(value)) throw new FormValidationError();
  return value;
}

export function readUrlList(input: Record<string, unknown>, key: string, maximumItems: number) {
  const value = input[key];
  ...
  const urls = value.map((item) => {
    if (typeof item !== "string" || item.length > 2_048) throw new FormValidationError();
    let parsed: URL;
    try {
...
export function readOptionalJsonObject(input: Record<string, unknown>, key: string) {
  const value = input[key];
  if (value === undefined || value === null) return null;
  if (typeof value !== "object" || Array.isArray(value)) throw new FormValidationError();
