...
import { AUTHORIZED_SIGNATURE_FONT_BASE64 } from "./assets/authorized-signature-font";
...
export async function loadAuthorizedSignatureFontBytes(
  encodedFont: string = AUTHORIZED_SIGNATURE_FONT_BASE64,
): Promise<Uint8Array> {
  const normalized = encodedFont.replace(/\s+/g, "");
  const bytes =
    normalized.length > 0 && /^[A-Za-z0-9+/]+={0,2}$/.test(normalized)
      ? Buffer.from(normalized, "base64")
      : Buffer.alloc(0);
  if (bytes.length === 0) throw new Error(AUTHORIZED_SIGNATURE_FONT_MISSING);
