...
import type { AdminProjectRequest } from "@growblic/contracts";
import { fetchAdminSubmissionPage } from "@/server/backend/backend-admin";
export const dynamic = "force-dynamic";
export default async function Page({ searchParams }: { searchParams: AdminPageSearchParams }) { const query = await readSubmissionPageQuery(searchParams, ENQUIRY_STATUSES); const result = await fetchAdminSubmissionPage<AdminProjectRequest>("project-requests", query); return <AdminShell><SubmissionListPage title="Project Requests" description="Start Project submissions, excluding price calculator leads." basePath="/project-requests/" items={result.items} pagination={result.pagination} search={query.search} status={query.status ?? ""} statuses={ENQUIRY_STATUSES} columns={[
