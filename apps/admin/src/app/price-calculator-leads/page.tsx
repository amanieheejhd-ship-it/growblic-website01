...
import type { AdminPriceCalculatorLead } from "@growblic/contracts";
import { fetchAdminSubmissionPage } from "@/server/backend/backend-admin";
export const dynamic = "force-dynamic";
export default async function Page({ searchParams }: { searchParams: AdminPageSearchParams }) { const query = await readSubmissionPageQuery(searchParams, ENQUIRY_STATUSES); const result = await fetchAdminSubmissionPage<AdminPriceCalculatorLead>("price-calculator-leads", query); return <AdminShell><SubmissionListPage title="Price Calculator Leads" description="Structured estimates generated through the price calculator." basePath="/price-calculator-leads/" items={result.items} pagination={result.pagination} search={query.search} status={query.status ?? ""} statuses={ENQUIRY_STATUSES} columns={[
