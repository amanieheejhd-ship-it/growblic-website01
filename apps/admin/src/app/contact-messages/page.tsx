...
import { listContactMessages } from "@/server/submissions/admin-submissions.repository";
export const dynamic = "force-dynamic";
export default async function Page({ searchParams }: { searchParams: AdminPageSearchParams }) { const query = await readSubmissionPageQuery(searchParams, ENQUIRY_STATUSES); const result = await listContactMessages(query); return <AdminShell><SubmissionListPage title="Contact Messages" description="Messages submitted through the public contact form." basePath="/contact-messages/" items={result.items} pagination={result.pagination} search={query.search} status={query.status ?? ""} statuses={ENQUIRY_STATUSES} columns={[
