...
import type { AdminMeetupRequest } from "@growblic/contracts";
import { fetchAdminSubmissionPage } from "@/server/backend/backend-admin";
export const dynamic = "force-dynamic";
export default async function Page({ searchParams }: { searchParams: AdminPageSearchParams }) { const query = await readSubmissionPageQuery(searchParams, ENQUIRY_STATUSES); const result = await fetchAdminSubmissionPage<AdminMeetupRequest>("meetup-requests", query); return <AdminShell><SubmissionListPage title="Meetup Requests" description="Consultation and meetup interest submissions." basePath="/meetup-requests/" items={result.items} pagination={result.pagination} search={query.search} status={query.status ?? ""} statuses={ENQUIRY_STATUSES} columns={[
