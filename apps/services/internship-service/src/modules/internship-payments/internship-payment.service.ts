...
import { generateInternshipInvoicePdf } from "@growblic/internship-shared";
import {
  asNonEmptyString,
  constantTimeEqual,
...
export class InternshipPaymentService {
  ...
  async completeDemoPayment(access: PaymentAccess) {
    ...
    if (payment.status === "PAID") return this.demoSuccess(payment);

    const gatewayPaymentId =
      payment.gatewayPaymentId ??
      `DEMO_${payment.gatewayOrderId.slice("demo_order_".length).toUpperCase()}`;
    ...
    return this.demoSuccess(refreshed);
  }

  private demoSuccess(payment: DatabaseClient) {
    const ready = this.trustedPaidInvoice(payment);
    if (!payment.gatewayPaymentId) throw new ConflictException();
    return {
      success: true,
          ...
          async (transaction: DatabaseClient) => {
            ...
            if (assignedCount === confirmationFields.length) {
              ...
              const dates = certificateDates(
                ...
                this.certificateReminderDays(),
              );
              await transaction.internshipCertificate.upsert({
                where: { paymentId: payment.id },
                create: {
                  publicReference: certificatePublicReference(
                    payment.paidAt.getUTCFullYear(),
                  ),
                  applicationId: payment.internshipApplicationId,
                  paymentId: payment.id,
                  durationDays: payment.selectedDuration,
                  paidAt: payment.paidAt,
                  ...dates,
  ...
  private async settlePaid(paymentId: string, gateway: RazorpayPaymentSnapshot, webhook?: { eventId: string; eventType: string }) {
    ...
    for (let attempt = 0; attempt < 5; attempt += 1) {
      try {
        ...
        const startDate = payment.internshipStartedAt ?? payment.joiningDate ?? paidAt;
        const computedDates = certificateDates(
          startDate,
          source.durationDays,
          this.certificateReminderDays(),
        );
        let confirmationSequence = payment.confirmationSequence;
        let confirmationYear = payment.confirmationYear;
        let confirmation = payment.confirmationReference;
        const needsOfferLetter =
          !payment.confirmationReference ||
          !payment.confirmationSequence ||
          !payment.confirmationYear ||
          !payment.confirmationIssuedAt ||
          !payment.joiningDate;
        if (needsOfferLetter) {
          confirmationYear = paidAt.getUTCFullYear();
          if (transaction.internshipConfirmationSequence?.upsert) {
            const sequence = await transaction.internshipConfirmationSequence.upsert({
              where: { year: confirmationYear },
              create: { year: confirmationYear, lastValue: 1 },
              update: { lastValue: { increment: 1 } },
            });
            confirmationSequence = sequence.lastValue;
          } else {
          ...
          confirmation = confirmationReference(confirmationYear, confirmationSequence);
        }
        const updated = await transaction.internshipPayment.update({
          ...
          data: {
            ...
            paymentMethod: gateway.method,
            paidAt,
            internshipStartedAt: startDate,
            expectedCompletionAt: computedDates.completionDate,
            certificateAvailableAt: computedDates.completionDate,
            joiningDate: payment.joiningDate ?? startDate,
            confirmationReference: confirmation,
            confirmationSequence,
            confirmationYear,
            confirmationIssuedAt: payment.confirmationIssuedAt ?? paidAt,
            offerLetterGeneratedAt: payment.offerLetterGeneratedAt ?? paidAt,
          },
        });
        ...
        await transaction.internshipCertificate.upsert({
          where: { paymentId: payment.id },
          create: {
            publicReference: certificatePublicReference(paidAt.getUTCFullYear()),
            applicationId: payment.internshipApplicationId,
            paymentId: payment.id,
          ...
          update: {
            joiningDate: computedDates.joiningDate,
            completionDate: computedDates.completionDate,
            reminderDueAt: computedDates.reminderDueAt,
            status: "PENDING_SKILLS",
          },
        });
        ...
        if (isUniqueConflict(error)) {
          const existingCertificate = await database.internshipCertificate.findUnique({
            where: { paymentId },
            select: { id: true },
          });
          if (existingCertificate) {
            const existingPayment = await database.internshipPayment.findUniqueOrThrow({
              where: { id: paymentId },
            });
            if (
              existingPayment.status === "PAID" &&
              existingPayment.gatewayPaymentId === gateway.id
            ) {
              return existingPayment;
            }
          }
          if (attempt < 4 && !existingCertificate) continue;
        }
        throw error;
      }
    }
  ...
  private certificateReminderDays() {
    const value = Number(process.env.INTERNSHIP_CERTIFICATE_REMINDER_DAYS ?? "5");
    return Number.isInteger(value) && value >= 1 && value <= 30 ? value : 5;
  }
  ...
  private async deliverInvoice(paymentId: string) {
    ...
    try {
      ...
      const invoiceFromEmail = requiredEnvironment("INVOICE_FROM_EMAIL");
      const invoiceAdminEmail = requiredEnvironment("INVOICE_ADMIN_EMAIL");
