...
import type { BackendConfig } from "../src/common/config/backend-config";
import { loadBackendConfig } from "../src/common/config/backend-config";
import {
...
describe("backend foundation", () => {
  it("returns liveness", async () => {
    ...
    assert.equal(response.body.service, "growblic-backend");
  });
  ...
  it("returns a generic safe response for an unhandled exception", async () => {
    const app = await createApp({ check: async () => undefined });
    const response = await request(app.getHttpServer()).get("/__foundation-test/error").expect(500);
