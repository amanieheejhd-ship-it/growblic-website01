...
async function withDemoEnvironment<T>(
  ...
  const previousEnabled = process.env.DEMO_PAYMENT_GATEWAY_ENABLED;
  process.env.NODE_ENV = environment;
  if (enabled === undefined) delete process.env.DEMO_PAYMENT_GATEWAY_ENABLED;
  else process.env.DEMO_PAYMENT_GATEWAY_ENABLED = enabled;
  try {
    return await run();
  } finally {
    if (previousEnvironment === undefined) delete process.env.NODE_ENV;
    else process.env.NODE_ENV = previousEnvironment;
    if (previousEnabled === undefined) delete process.env.DEMO_PAYMENT_GATEWAY_ENABLED;
    else process.env.DEMO_PAYMENT_GATEWAY_ENABLED = previousEnabled;
  }
}
...
describe("internship payment service gate", () => {
  ...
  it("authenticates and idempotently completes a demo payment with one invoice", async () => {
    ...
    await withDemoEnvironment("test", "true", async () => {
      ...
      assert.equal(database.payment().confirmationReference, null);
