...
export default function GrowblicAIChat() {
  ...
  return (
    <>
      ...
      {open ? (
        <div className="pointer-events-none fixed inset-0 z-[90] font-sans">
          <div className="absolute inset-0 bg-slate-950/[0.03] backdrop-blur-[1px]" />

          <section
            aria-label="Growblic Assistant chat drawer"
            className="pointer-events-auto absolute bottom-2 right-2 flex h-[calc(100dvh-16px)] w-[calc(100vw-16px)] animate-[growblicSheetIn_260ms_cubic-bezier(0.16,1,0.3,1)] flex-col overflow-hidden rounded-[30px] border border-white/80 bg-white/92 shadow-[0_32px_100px_rgba(15,23,42,0.24),0_12px_44px_rgba(14,165,233,0.12)] ring-1 ring-slate-900/[0.04] backdrop-blur-2xl sm:bottom-3 sm:right-3 sm:top-3 sm:h-auto sm:w-[440px] sm:animate-[growblicDrawerIn_260ms_cubic-bezier(0.16,1,0.3,1)] sm:rounded-[32px] lg:w-[456px]"
          >
            <header className="relative overflow-hidden border-b border-slate-200/70 bg-[linear-gradient(135deg,rgba(255,255,255,0.98),rgba(239,248,255,0.92))] px-5 py-5 sm:px-6">
              ...
              <div className="relative flex items-start justify-between gap-4">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="inline-flex items-center gap-1.5 rounded-full border border-sky-100 bg-sky-50/90 px-2.5 py-1 text-[11px] font-extrabold text-sky-700 shadow-sm">
              ...
              <div className="relative mt-5 flex items-center gap-3 rounded-2xl border border-sky-100/80 bg-white/82 p-3 shadow-[0_12px_34px_rgba(15,23,42,0.06)] ring-1 ring-white/80 backdrop-blur-xl">
            ...
            >
              {messages.map((item, index) => (
                ...
                >
                  <div
                    className={`max-w-[88%] px-4 py-3 text-sm font-semibold leading-6 shadow-sm ${
                      item.role === "user"
            ...
            <div className="border-t border-slate-200/70 bg-white/90 px-4 py-4 shadow-[0_-18px_46px_rgba(255,255,255,0.80)] backdrop-blur-2xl sm:px-5">
              <form
                onSubmit={handleSubmit}
                className="flex items-center gap-2 rounded-[24px] border border-slate-200/90 bg-white p-1.5 shadow-[0_14px_38px_rgba(15,23,42,0.08)] ring-1 ring-white/80 transition focus-within:border-sky-200 focus-within:ring-4 focus-within:ring-sky-100"
              >
                <button
                  type="button"
                  aria-label="Voice input unavailable"
                  disabled
                  className="grid h-10 w-10 shrink-0 cursor-not-allowed place-items-center rounded-full text-slate-300"
                >
